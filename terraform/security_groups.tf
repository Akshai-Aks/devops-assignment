...
# App EC2: accepts traffic only from ALB on app port; SSH from within VPC
resource "aws_security_group" "app" {
  name        = "${var.project}-app-sg"
  description = "Allow app port from ALB; SSH from VPC"
  vpc_id      = aws_vpc.main.id
  ...
  tags = merge(local.tags, { Name = "${var.project}-app-sg" })
}

# RDS: accepts PostgreSQL only from app EC2 instances
